...
def create_frame_tasks(w, task):
    ...
    checkbox = ctk.CTkCheckBox(frame, variable=checkbox_var)

    checkbox.pack(side='left', padx=10)

    text_label = ctk.CTkLabel(frame, text=task, font=ctk.CTkFont(weight='bold' if task['priority'] == 'Высокий' else 'normal'))
    text_label.pack(side='left', padx=10)

    category_label = ctk.CTkLabel(frame, text=task['category'])
    category_label.pack(side='left', padx=10)

    delete_btn = ctk.CTkButton(frame, text='Удалить', fg='red', hover_color='darked')
